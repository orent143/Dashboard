<template>
  ...
  export default {
    ...
    methods: {
      ...
      handleSubmit() {
        const newUser = {
          user_id: Math.floor(Math.random() * 100000), // dummy ID
        ...
        if (this.formData.role === 'student') {
          newUser.degree = this.formData.degree.trim();
        }
  
        this.$emit('user-added', newUser);
        this.toast.success(`${newUser.name} has been added successfully as a ${newUser.role}!`);
        this.closeModal();
      },
      closeModal() {
        this.$emit('close');
